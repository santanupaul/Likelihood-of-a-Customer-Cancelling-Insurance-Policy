...
if (make.plots){
  ...
  plot(log(lr.reg.path$lambda),
       ...
       main="Validation classification accuracy")
}
...
AUC #73.38% (With all vars). 72.37% (with reduced vars)
...
AUC <- performance(pred_input,"auc")
AUC #68.99% (with all vars). 66.9% (with reduced vars)
...
AUC <- performance(pred_input,"auc")
AUC #72.84% (with all vars). 70.91% (with reduced vars)
...
fwrite(cstat_all, 'L1 Regularization AUCs.csv')